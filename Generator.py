...
def DrawLeaderBoard(leaderboard_data, leaderboardName, counter):
    ...
    fontBig = ImageFont.truetype(
        ".\Assets\Fonts\Archivo_Black\ArchivoBlack-Regular.ttf", 40)
    fontMedium = ImageFont.truetype(
        ".\Assets\Fonts\Archivo_Black\ArchivoBlack-Regular.ttf", 30)
    fontSmall = ImageFont.truetype(
        ".\Assets\Fonts\Archivo_Black\ArchivoBlack-Regular.ttf", 25)

    # date
    image_editable.text((620, 115), str(today),
                        ("#FFFFFF"), font=fontMedium)
    ...
    for index in range(len(leaderboard_data)):
        ...
        # ranking
        if ranking > 9:
            image_editable.text((85, 300 + row_y), str(ranking),
                                ("#FFFFFF"), font=fontBig)
        else:
            image_editable.text((100, 300 + row_y), str(ranking),
                                ("#FFFFFF"), font=fontBig)

        # playername
        image_editable.text((220, 275 + row_y+offset*2), item.name,
                            ("#FFFFFF"), font=fontBig)
        # playerdeck
        image_editable.text((220, 330 + row_y), item.deck,
                            ("#FFFFFF"), font=fontSmall)
        # playerscore
        image_editable.text((930, 300 + row_y),
                            str(item.score), ("#FFFFFF"), font=fontBig)
